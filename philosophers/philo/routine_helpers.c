...
/*   Updated: 2025/05/07 13:04:16 by valeriia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_check_death(unsigned int philo_number)
{
	static t_monitor	*saved_monitor;
	size_t				i;
		...
		- saved_monitor->philos[philo_number - 1].last_meal)
	{
		...
		pthread_mutex_unlock(&(saved_monitor->monitor_mutex));
		ft_print_state(saved_monitor->philos[philo_number - 1].number);
		pthread_mutex_lock(&(saved_monitor->monitor_mutex));
		i = 0;
